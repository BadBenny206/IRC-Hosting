...
class TreasuryRegsXMLParser:
    ...
    def _parse_section(self, section_elem: ET.Element, source_name: str) -> Optional[RegulationNode]:
        ...
        # Process child elements in document order to maintain relationships
        # (P elements followed by associated EXTRACT/EXAMPLE elements)
        paragraphs = []
        elements_with_tags = []  # Track which element type each paragraph came from
        extract_text = ""  # Store EXTRACT separately to append to (b)(8)(i)
        
        for child in section_elem:
            if child.tag == "P":
                p_text = self._extract_text(child)
                if p_text:
                    paragraphs.append(p_text)
                    elements_with_tags.append((child, "P", len(paragraphs) - 1))
            elif child.tag == "EXTRACT":
                # Store EXTRACT separately 
                extract_text = self._format_extract_element(child)
            elif child.tag == "EXAMPLE":
                example_text = self._format_example_element(child)
                if example_text:
                    paragraphs.append(example_text)
                    elements_with_tags.append((child, "EXAMPLE", len(paragraphs) - 1))
        ...
        # Build para_tuples using tracked indices
        para_tuples = []
        for elem, elem_type, idx in elements_with_tags:
            if elem_type == "P":
                is_italic = self._p_has_italic_marker(elem)
            else:
                # EXAMPLE elements don't have italic markers
                is_italic = False
            para_tuples.append((paragraphs[idx], is_italic))
        
        node.subsections = self._build_flat_subsections(para_tuples)
        
        # If there's an EXTRACT, append it to (b)(8)(i) as a special node
        if extract_text:
            def find_b8i(subs):
                for sub in subs:
                    ...
                    if "(8)" in sub.displayId:
                        ...
                        for child in sub.subsections:
                            if "(i)" in child.displayId:
                                # Found (b)(8)(i) - append table as a sibling of its content
    ...
    def _format_extract_element(self, extract_elem: ET.Element) -> str:
        """Extract EXTRACT element and format as [TABLE] for HTML rendering."""
        # Collect all FP elements (each represents a row)
        fp_elements = extract_elem.findall(".//FP")
        if not fp_elements:
            # Fallback: just get all text
            text_parts = []
            for elem in extract_elem.iter():
        ...
        # Build table from FP elements - each FP is a row
        rows = []
        for fp in fp_elements:
            # Extract all text from this FP element
            text_content = self._extract_text(fp).strip()
            if text_content:
                rows.append(text_content)
        
        # Format as [TABLE] ... [END TABLE] markup for rendering
        if rows:
            table_text = "\n".join(rows)
            return f"\n[TABLE]\n{table_text}\n[END TABLE]"
    ...
    def _format_example_element(self, example_elem: ET.Element) -> str:
        """Format EXAMPLE element with heading and content.
        
        EXAMPLE elements have:
        - <HD SOURCE="HED"> for the example heading
        - <P> child elements with the example content
        """
        parts = []
        
        # Get the heading
        heading = example_elem.findtext("HD", "").strip()
        if heading:
            parts.append(heading)
        
        # Get all paragraph content
        for p_elem in example_elem.findall("P"):
...
"""
    ...
    def _extract_text(self, elem: ET.Element) -> str:
        ...
        text = " ".join(str(part).strip() for part in text_parts if part)
        if not text:
            return ""
        try:
