...
"""
    ...
    def _extract_direct_content(self, node: ET.Element) -> str:
        """Extract only the node's own text, not text from nested legal children."""
        content_parts: List[str] = []

        for child in node:
            child_name = self._local_name(child)
            if child_name in STRUCTURAL_TAGS or child_name in {"num", "heading"}:
                continue
            ...
            if child_name == "continuation":
                ...
                if cont_text:
                    content_parts.append(cont_text)
